...
namespace Fuzzy
{
    public static class IFuzzArrayExtensions
    {
        ...
        public static FuzzyArray<T> Array<T>(this IFuzz fuzzy, Func<Fuzzy<T>> createElement, Length length = default) =>
            fuzzy.Array(() => (T)createElement(), length);

        public static FuzzyArray<T> Array<T>(this IFuzz fuzzy, Func<FuzzyRange<T>> createElement, Length length = default) where T : struct, IComparable<T> =>
